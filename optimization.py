...
from Cyclone.create_windowpanes import create_arbitrary_windowpanes, maximum_coil_radius, clean_components, rotate_windowpane_shapes, planar_vector_list
from Cyclone.helper_functions import rz_surf_to_xyz, rz_normal_to_xyz


def change_arbitrary_windowpanes(ntoroidalcurves, npoloidalcurves, nfp, stellsym, sin_components, cos_components, 
                                 winding_surface_function = None, order=5):
    curve = None
    for i in range(ntoroidalcurves):
        ...
        for j in range(npoloidalcurves):
            ...
            components = np.array([[0.0]*3 for i in range(2*order)])
            if normaltowinding:
            ...
            for s in range(order):
                components[2*s] = sin_components[s][0]*planar_vectors[0] + sin_components[s][1]*planar_vectors[1]
            # Set the cos components
            for c in range(order):
                components[2*c+1] = cos_components[c][0]*planar_vectors[0] + cos_components[c][1]*planar_vectors[1]
            curves[npoloidalcurves*i+j].x = list(components.T.flatten())
            curves[npoloidalcurves*i+j].x = curves[npoloidalcurves*i+j].x
...
dofs = list(np.append(sin_components.flatten(), cos_components.flatten()))

print(sin_components)
print(cos_components)

def fun(dofs):
    J = JF.J()
    dofsarr = np.array(dofs)
    sin_components = dofsarr[:2*order].reshape(order,2)
    cos_components = dofsarr[2*order:].reshape(order,2)
    change_arbitrary_windowpanes(ntoroidalcoils, npoloidalcoils, 2, True, sin_components, cos_components,
                                 winding_surface_function = winding_surface_function, order=order)
    Bbs = bs.B().reshape((nphi, ntheta, 3))
    BdotN = np.sum(Bbs * s.unitnormal(), axis=2) / np.linalg.norm(Bbs, axis=2)
    BdotNsum = np.sum(np.abs(BdotN))
    J+=0.01*BdotNsum
    return J
...
for _ in range(sample_size):
    start_dofs = list(np.random.uniform(-0.5, 0.5, len(dofs)))
    dofs = start_dofs
    res = minimize(fun, dofs)
    J = fun(dofs)
    if J < J_best:
